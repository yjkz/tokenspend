  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-400 mt-0.5">{data.projectPath || data.projectDir} &middot; {formatTime(data.firstTimestamp, lang === 'zh' ? 'zh-CN' : 'en-US')} — {formatTime(data.lastTimestamp, lang === 'zh' ? 'zh-CN' : 'en-US')}</p>
      </div>
            ...
            <tbody>
              {data.requests.map((r, i) => (
                <tr key={i} className="border-b border-pink-100">
                  ...
                  <td className="py-1.5 pr-3 text-gray-500 text-xs">{formatTime(r.timestamp, lang === 'zh' ? 'zh-CN' : 'en-US')}</td>
